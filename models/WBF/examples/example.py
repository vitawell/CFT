...
def example_wbf_2_models(det1, det2, img, iou_thr=0.55, draw_image=True):
    ...
    boxes_list = [det1_boxes, det2_boxes]
    scores_list = [det1_score, det2_score]
...
def example_wbf_1_model(det1, img, iou_thr=0.55, draw_image=True):
    """
    ...
    # Arrange datas
    boxes_list = np.array(det1[:,:4])
    scores_list = np.array(det1[:,4])
